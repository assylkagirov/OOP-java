...
public class Database implements Serializable {
    ...
    protected static Vector<UniNews> UniNews = new Vector<>();
    protected static Vector<Lesson> lessons = new Vector<>();
    protected static Vector <Student> dormStudents = new Vector<>();
    ...
    public static Vector<UniNews> getUniNews() {
        return UniNews;
    }

    public static void setUniNews(Vector<UniNews> UniNews) {
        Database.UniNews = UniNews;
    }
    ...
    public static Vector<Student> getDormStudents() {
        return dormStudents;
    }

    public static void setDormStatus(Vector<Student> dormStudents) {
        Database.dormStudents = dormStudents;
    }
    ...
    private static void saveUniNews() {
        try (ObjectOutputStream oot = new ObjectOutputStream(new FileOutputStream("UniNews.txt"))) {
            oot.writeObject(UniNews);
            oot.flush();
        }
        catch (IOException e) {
            System.err.println("UniNews.txt: IOException");
        }
    }
    ...
    private static void saveDormStudents() {
        try (ObjectOutputStream oot = new ObjectOutputStream(new FileOutputStream("dormStudents.txt"))) {
            oot.writeObject(dormStudents);
            oot.flush();
        }
        catch (IOException e) {
            System.err.println("dormStudents.txt: IOException");
        }
    }
    ...
    private static void loadUniNews() {
        try {
        	fis = new FileInputStream("UniNews.txt");
        	oin = new ObjectInputStream(fis); 
            UniNews = (Vector<UniNews>) oin.readObject();
            oin.close();
            fis.close();
        }
        catch (ClassNotFoundException e) {
            UniNews = new Vector<>();
            System.err.println("UniNews.txt: ClassNotFoundException");
        }
        catch (IOException e) {
            UniNews = new Vector<>();
            System.err.println("UniNews.txt: IOException");
